...
namespace UnitTests
{
    ...
    public class BigIntegerTests
    {
        ...
        public void Log()
        {
            ...
            for (int i = 0; i < significantNumbers.Length; i++)
            {
                for (int j = 0; j < baseValues.Length; j++)
                {
                    ...
                    Assert.AreEqual(expected, Convert(actual), string.Format("Computing Log({0}, {1})", significantNumbers[i], baseValues[j]));
                }
            }
        }
        ...
        private System.Numerics.BigInteger Convert(double value)
        {
